...
/* Get the landmark  data from map 
 * @param map The map data
 * @param id The id of landmark in map data
 * @output landmark 
 */
inline LandmarkObs get_landmark_map(double id,const Map &map)
{
	...
	for (int i=0;i < map.landmark_list.size();i++){
		...
		if (id == map.landmark_list[i].id_i){

			landmark_map.id = id ;
			landmark_map.x = map.landmark_list[i].x_f;
			landmark_map.y = map.landmark_list[i].y_f;
